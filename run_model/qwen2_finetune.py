...
os.environ["WANDB_PROJECT"] = "recavef"
...
def preprocess(
    ...
    roles = {"human": "<|im_start|>human", "gpt": "<|im_start|>gpt"}

    im_start = tokenizer.im_start_id
    im_end = tokenizer.im_end_id
    nl_tokens = tokenizer("\n").input_ids
    _system = tokenizer("system").input_ids + nl_tokens
    _human = tokenizer("human").input_ids + nl_tokens
    _assistant = tokenizer("gpt").input_ids + nl_tokens
    ...
    for i, source in enumerate(sources):
        ...
        input_id += system
        target += (
            [im_start] + [IGNORE_TOKEN_ID] *
            (len(system) - 3) + [im_end] + nl_tokens
        )
...
def train():
    ...
    tokenizer.add_special_tokens(
        {"additional_special_tokens": all_add_special_tokens})
    ...
    tokenizer.im_start_id = tokenizer.convert_tokens_to_ids(["<|im_start|>"])[
        0]
    tokenizer.im_end_id = tokenizer.convert_tokens_to_ids(["<|im_end|>"])[0]

    tokenizer.pad_token = tokenizer.eos_token
    tokenizer.eod_id = tokenizer.im_end_id
